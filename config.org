...
counsel is a companion package for Ivy
#+begin_src emacs-lisp
  (use-package counsel
    :ensure t
    :init (counsel-mode))
...
Note: Org-Roam V2 inserted a PROPERTY header to all files, which doesn't display well in Deft. But luckily Org-Roam V2 can replace Deft, so this section has become obsolete. I will remove it after a little while.
...
#+end_src
** Customized title
Filter out headers in Deft's title display.
#+begin_src emacs-lisp
...
Notes about V2:
- Use org-roam-setup instead of org-roam-mode to start up
...
This matches things like #+title: or #+TITLE:
  ...
  This matches the "drawer" introduced in Org Roam V2.
  Removes all lines that begin with a colon
