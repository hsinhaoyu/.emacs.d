  ...
  (use-package lsp-ltex
    ...
    :hook ((text-mode org-mode markdown-mode)
           . (lambda ()
               ...
               (lsp)))            ; or (lsp-deferred)
    :init
    ;; Language for grammar checking
    (setq lsp-ltex-language "en-AU")
    ;; Tell lsp-ltex where the LS lives (NOTE the name: lsp-ltex-ls-path)
...
(with-eval-after-load 'org-capture
  ...
  ;; 1. Target function: open the org file and move point to the *top*
  ;;    of the posts list, so capture PREPENDS.
  (defun my/org-hugo-micro-prepend-target ()
    "Capture target for Hugo micro-posts, prepending under a heading."
    (find-file "~/Library/Mobile Documents/com~apple~CloudDocs/sites/try_ox_hugo/quickstart/content-org/app-posts.org")
    ;; Go to the heading under which you store the micro posts.
    (goto-char (point-min))
    (unless (re-search-forward "^\\* +Micro posts\\b" nil t)
      ;; If it doesn't exist, create it at end of file.
      (goto-char (point-max))
      (insert "\n* Micro posts\n"))
    ;; Go to that heading and place point just after it, before any children.
    (org-back-to-heading t)
    (org-show-subtree)
    (forward-line 1))  ;; insertion point = first line after heading → prepend

  ;; 2. Template function: builds the subtree text with the properties you want.
  (defun my/org-hugo-micro-post-template ()
    "Return an org subtree string for a new Hugo micro post."
    (let* ((title (read-from-minibuffer "Micro post title: "))
           (slug  (org-hugo-slug title))
           ;; YYYYMMDDHHMM
           (ts    (format-time-string "%Y%m%d%H%M"))
           (fname (format "%s_%s" ts slug)))
      (mapconcat
       #'identity
       (list
        (concat "* TODO " title)
        ...
        ;; You asked for `"tag1" "tag2"`. ox-hugo is fine with this format
        ;; for multi-word tags; you can change these strings as you like.
  ...
  ;; 3. Capture template using the target + template above.
  (add-to-list 'org-capture-templates
               '("m" "Hugo micro post (prepend)"
                 entry
                 (function my/org-hugo-micro-prepend-target)
                 (function my/org-hugo-micro-post-template))))
#+end_src
