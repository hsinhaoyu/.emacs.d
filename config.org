...
* Loading
I use this directory for my personal elisp code.
...
Press "a" to open a dir in the same buffer.
Press "^" to go up a dir in the same buffer.
#+begin_src emacs-lisp
  (put 'dired-find-alternate-file 'disabled nil)

  (with-eval-after-load 'dired
    ;; "a" = open dir in same buffer
    (define-key dired-mode-map (kbd "a") #'dired-find-alternate-file)

    ;; "^" = go up in same buffer
...
Note: I decided to not to set =org-agenda-files= because it causes performance problems. I don't use agenda for the time being
  #+begin_src emacs-lisp
...
#+end_src
* Writing - =languagetool= and =LSP=
I used to use =flycheck-languagetool=, but I have replaced it with =lsp-ltex=.

Components of this setup:
-  Language server (LSP): [[https://github.com/valentjn/ltex-ls][LTEX LS]]:
  I downloaded it from the project's github. It's in =/Applications/ltex-ls-16.0.0/bin/ltex-ls=
  =lsp-ltex= maintains its own version of LanguagetTool. It's not the same one used in the LanguageTool for Desktop app.
- The emacs [[https://github.com/emacs-languagetool/lsp-ltex][lsp-ltex]] package
  Sends the diagnostics (via the LSP) to Flycheck
- Flycheck
  The UI element to displaying the checks
  
** Setting up =lsp-ltex=
This block forces emacs to use the homebrew Java, not the Java that comes with macOS.
Also, Java has some limits on XML entity size. Disable that check.
#+begin_src emacs-lisp
  ;; Make Emacs use the same PATH as your shell (including Homebrew's Java)
  (use-package exec-path-from-shell
    :ensure t
    :config
    (exec-path-from-shell-initialize))

  ;; Explicitly set JAVA_HOME and exec-path for Java
  (setenv "JAVA_HOME" "/opt/homebrew/opt/openjdk")
  (add-to-list 'exec-path "/opt/homebrew/opt/openjdk/bin")

  ;; Make sure Emacs passes this JVM option to ltex-ls (and other Java tools)
  (let ((opt "-Djdk.xml.totalEntitySizeLimit=0"))
    (setenv "JAVA_TOOL_OPTIONS"
          (string-join
           (seq-filter #'identity
                       (list (getenv "JAVA_TOOL_OPTIONS") opt))
           " ")))
#+end_src

#+begin_src emacs-lisp
  ;; Use Flycheck for LSP diagnostics (instead of Flymake)
  (with-eval-after-load 'lsp-mode
    (setq lsp-diagnostics-provider :flycheck))

  (defun my/lsp-ltex-maybe-start ()
    "Start lsp-ltex only in real file-visiting Org/Text/Markdown buffers."
    (when buffer-file-name
      (require 'lsp-ltex)
      (lsp-deferred)))  ;; non-blocking

  (use-package lsp-ltex
    :ensure t
    :hook ((text-mode org-mode markdown-mode)
           . my/lsp-ltex-maybe-start)
    :init
    (setq lsp-ltex-language "en-AU")
    (setq lsp-ltex-ls-path "/Applications/ltex-ls-16.0.0"))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'flycheck
    ;; In Org/Text/Markdown, only use the LSP checker.
    ...
    ;; If you ever had flycheck-languagetool, keep it disabled for safety.
    (add-to-list 'flycheck-disabled-checkers 'languagetool))
#+end_src

** User experience (disabled)
#+begin_comment
#+begin_src emacs-lisp
(use-package lsp-ui
  :ensure t
  ...
  :custom
  ;; Show a small popup with info / diagnostics at point
  (lsp-ui-doc-enable t)
  (lsp-ui-doc-show-with-cursor t)  ;; show when you move cursor
  (lsp-ui-doc-show-with-mouse t)   ;; and/or when you hover with mouse

  ;; Show diagnostics in the sideline (optional, but handy)
  (lsp-ui-sideline-enable t)
  (lsp-ui-sideline-show-diagnostics t)
  (lsp-ui-sideline-show-hover nil)) ;; don’t spam hover text in sideline
#+end_src
#+end_comment

* Blogging with Hugo

Install =ox-hugo= because I want to automate Hugo posting with =org=.
...
** Custom Functions for posting
#+begin_src emacs-lisp
  (defun my-hugo-slug (s)
    "Generate a simple Hugo slug from S."
    (let* ((s (downcase s))
           (s (replace-regexp-in-string "[^a-z0-9]+" "-" s))
           (s (replace-regexp-in-string "^-\\|-+$" "" s)))
      s))

  (defun my-hugo-generate-fname (title)
    "Generate a timestamped Hugo filename from TITLE.
  If TITLE is empty, return only the timestamp."
    (let ((ts (format-time-string "%Y%m%d%H%M")))
      (if (or (null title) (string-empty-p title))
          ts
        (format "%s_%s" ts (my-hugo-slug title)))))

  (defun hhyu-hugo-insert-post ()
    "Insert a new Hugo post subtree at point.

  Prompts for:
    • Hugo section (\"micro\" or \"post\")
    • Post title (optional)

  If title is empty:
    - Heading title becomes \"untitled\"
    - EXPORT_FILE_NAME = timestamp only."
    (interactive)
    (let* ((section (completing-read
                     "Hugo section: "
                     '("micro" "post")
                     nil t nil nil "micro"))
           (raw-title (read-from-minibuffer "Post title (optional): "))
           (title     (if (string-empty-p raw-title)
                          "untitled"
                        raw-title))
           (fname     (my-hugo-generate-fname raw-title))) ;; use raw title here for filename rule
      (insert
       (format "** TODO %s
  :PROPERTIES:
  :EXPORT_FILE_NAME: %s
  :EXPORT_HUGO_SECTION: %s
...
#+begin_src emacs-lisp
  ...
  (defun hhyu/hugo-resize-image (filename)
    "Resize image FILENAME to at most 1600px on the long side.
  Does nothing if ImageMagick 'mogrify' is not available."
    (when (and (executable-find "mogrify")
               (string-match-p (image-file-name-regexp) filename))
      (call-process "mogrify" nil nil nil
                    "-resize" "1600x1600>"  ;; shrink only, never enlarge
                    "-strip"                ;; remove metadata
                    "-quality" "85"         ;; JPEG quality
                    filename)))

  (defun hhyu-hugo-insert-image (file)
    "Copy an image FILE into ./images next to the current Org file,
  resize it, and insert an Org link suitable for ox-hugo.

  Resulting link looks like [[file:images/NAME.ext]].

  When exported with ox-hugo (with #+hugo_base_dir: ../),
  Hugo will see the image under static/images/NAME.ext and serve it
  at /images/NAME.ext."
    (interactive
     (list (read-file-name "Image file: " "~/Desktop/" nil t)))
    (unless (and buffer-file-name (derived-mode-p 'org-mode))
      (user-error "This command must be run in an Org buffer visiting a file"))
    (let* ((org-dir     (file-name-directory buffer-file-name))
           (images-dir  (expand-file-name "images" org-dir))
           (ext         (downcase (or (file-name-extension file) "")))
           (base        (file-name-base file))
           ;; timestamp + cleaned base name, to avoid collisions
           (ts          (format-time-string "%Y%m%d-%H%M%S_"))
           (safe-base   (replace-regexp-in-string "[^a-zA-Z0-9_-]" "-" base))
           (new-name    (if (string-empty-p ext)
                            (concat ts safe-base)
                          (format "%s%s.%s" ts safe-base ext)))
           (target      (expand-file-name new-name images-dir)))
      ;; Ensure ./images exists
      (make-directory images-dir t)
      ;; Copy the file
      (copy-file file target t)
      ;; Resize in-place (if mogrify is available)
      (hhyu/hugo-resize-image target)
      ;; Insert Org link relative to the Org file
      (insert (format "[[file:images/%s]]" new-name))))
#+end_src
* Music (lilypond)
...
I tried paredit for structured editing of LISP/Scheme code, but I decided to disable it
