...
#+end_src
* Scratch buffer
The scratch buffer is a convenient to display a cheat sheet.
#+begin_src emacs-lisp
...
I decided to use the terminal mode emacs for the unix setting for visual editors. It seems to be fine.
...
if [[ $socket_file == "" ]]; then
    ...
    # I am not sure what I need --chdir $PWD for emacs to load the file from the working directory. But if I didn't add it, emacs starts with $HOME
